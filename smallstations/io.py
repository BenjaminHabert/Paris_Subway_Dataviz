...
def inventory():
    """logging.debug the list of files in the directory (goes into subfolders).
    Example
    -------
    >>> inventory()
    raw/
    ...
    for root, dirs, files in os.walk(DATA_FOLDER):
        ...
        for file in files:
            logging.debug(os.path.join(local_root, file))
...
def save_dataframe(df, relative_path):
    ...
    logging.debug('Saving dataframe to: ' + absolute_path)
    if extension == '.csv':
...
def load_dataframe(relative_path):
    ...
    logging.debug('Loading dataframe from: ' + absolute_path)
    if extension == '.csv':
